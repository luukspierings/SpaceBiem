...
namespace spacebiem
{
    PlanetEntity::PlanetEntity(float x, float y, Color color, float w, float h, const string& texture, const string& borderTexture, int pScoreBonus, const string& pName, bool scoreGiven)
    {
        ...
        addComponent("text", new TextComponent(Fonts::Roboto(), to_string(pScoreBonus), { 255,255,255, 150 }, true, w / 2.f, h / 2.f + 20.f, 10u, scoreGiven));
        addComponent("text", new TextComponent(Fonts::Roboto(), pName, { 255,255,255,255 }, true, w / 2.f, h / 2.f - 15.f, 10u, scoreGiven, "name"));
