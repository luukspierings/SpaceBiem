...
namespace spacebiem
{
    ...
    void UniverseBuilder::build(std::shared_ptr<EntityManager> entityManager, bool newGame, string fileName, bool focused)
    {
        ...
        for (auto& o : levelMap)
        {
            ...
            if (type == "player") {
                int id = entityManager->addEntity(new PlayerEntity(stoi(xPos), stoi(yPos), Color::White(), stoi(width), stoi(height), 1.f, focused));
                ...
                entityManager->addEntity(new MummieAIEntity(stoi(xPos), stoi(yPos), Color::White(), stoi(width), stoi(height)));
            }
            else if (type == "aisnowman") {
                entityManager->addEntity(new SnowmanAIEntity(stoi(xPos), stoi(yPos), Color::White(), stoi(width), stoi(height)));
            }
