...
namespace spacebiem
{
    UniverseGenerator::UniverseGenerator()
    {
        ...
        LOG("DIFFICULTY VECTOR LENGTH: %s", to_string(difficultyBelt["normal"]["outer"].size()).data());
    }
    ...
    void UniverseGenerator::addPlanetarySystem(int level, int beltCount, int middleX, int middleY, int sunR, int beltMargin, int beltW, bool withWormHole)
    {
        ...
        while (beltCount > 0) {
            ...
            LOG("GETTING MINPR / MAXPR / MINPMARGIN");

            LOG("MINPR DIFFICULTY: %s", currentDifficulty.data());
            LOG("MINPR BELTTYPE: %s", beltType.data());

            auto minPRvector = difficultyBelt[currentDifficulty][beltType];

            LOG("MINPR VECTOR LENGTH: %s", to_string(minPRvector.size()).data());

            auto minPRstr = difficultyBelt[currentDifficulty][beltType][0];

            LOG("MINPR: %s", minPRstr.data());

            float minPR = static_cast<float>(atof(minPRstr.data()));
            ...
            LOG("GETTING DIFFICULTYBELT INPUT");
            ...
            LOG("CALCULATING PROBABILITY TOKENS");
    ...
    void UniverseGenerator::addBelt(int middleX, int middleY, int minR, int maxR, float minPR, float maxPR, float minPMargin, vector<string> planetProbability)
    {
        ...
        while (startAngle + (100 - angleMinIncr) > newAngle) {
            ...
            LOG("CALCULATING BELT PLANET ANGLE");
