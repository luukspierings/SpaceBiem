...
namespace spacebiem
{
    ...
    void DifficultyScene::created()
    {
       ...
       /* int overlayId = addEntityExtra<SpriteEntity>([](Entity* entity)
        ...
        addEntity(new SpriteEntity("textures/choose_difficulty.png", wW / 2 - 175, 130, Color::White(), -1, -1));
        ...
        if (progress[Difficulty::NORMAL]) {
            LOG("DIFFICULTY NORMAL READY");
            normal = [](StateManager* manager)
            {
                auto nextAvail = static_cast<Difficulty>(static_cast<int>(Difficulty::NORMAL) + 1);
                manager->getSceneManager().navigateTo<StoryScene>(Difficulty::NORMAL);
            };
        }

        if (progress[Difficulty::CHALLENING]) {
            LOG("DIFFICULTY CHALLENGING READY");
            challenging = [](StateManager* manager)
            {
                auto nextAvail = static_cast<Difficulty>(static_cast<int>(Difficulty::NORMAL) + 1);
                manager->getSceneManager().navigateTo<StoryScene>(Difficulty::CHALLENING);
            };
        }

        if (progress[Difficulty::EXPERT]) {
            LOG("DIFFICULTY EXPERT READY");
            expert = [](StateManager* manager)
            {
                auto nextAvail = static_cast<Difficulty>(static_cast<int>(Difficulty::NORMAL) + 1);
                manager->getSceneManager().navigateTo<StoryScene>(Difficulty::EXPERT);
            };
        }


        addEntity(new ButtonUIEntity(x - 75, beginY + (0 * incr), Color{ 22, 94, 22 }, buttonTextColor, buttonSize, "Normal", buttonTexture, normal));

        addEntity(new ButtonUIEntity(x - 75, beginY + (1 * incr), Color{ 188, 103, 0 }, buttonTextColor, buttonSize, "Challenging", buttonTexture, challenging));

        addEntity(new ButtonUIEntity(x - 75, beginY + (2 * incr), Color{ 135, 25, 25 }, buttonTextColor, buttonSize, "Expert", buttonTexture, expert));

        beginY += 40;
        addEntity(new ButtonUIEntity(x - 75, beginY + (3 * incr), buttonColor, buttonTextColor, buttonSize, "Menu", buttonTexture,
