...
namespace spacebiem
{
    ...
    void LevelEditorScene::created()
    {
        ...
        // menu background
        addEntity(new SpriteEntity("textures/rectangle.png", wW - menuSize, 0, Color{ 0, 0, 0, 150 }, menuSize, wH, 0u, "entity_menu"));
        ...
        addEntity(new ButtonUIEntity((wW) - (buttonSize.width) - 20, wH - buttonSize.height - 20, buttonColor, buttonTextColor, buttonSize, "Back to slots", buttonTexture,
            [this](StateManager* manager)
        {
            manager->getSceneManager().navigateTo<LevelLoadScene>();
        }));

        addEntity(new SpriteEntity("textures/kliko.png", wW - menuSize + 30, wH - buttonSize.height - 50, Color::White(), 80, 80, 200u, "kliko"));

        addEntity(new CameraEntity(wW/2, wH/2, "_placed"));
    ...
    void LevelEditorScene::fillMenuEntitySpace() {
        ...
        if (earthGone) addEntity(new PlanetEarthEntity(column1X, row1Y, Colors::EarthAtmosphere(), planetWidth, planetWidth, 0, 10.f, nameGenerator.getName()));
        if (moonGone) addEntity(new PlanetMoonEntity(column2X, row1Y, Colors::MoonAtmosphere(), planetWidth, planetWidth, 10.f, nameGenerator.getName()));
        if (sandGone) addEntity(new PlanetSandEntity(column1X, row2Y, Colors::SandAtmosphere(), planetWidth, planetWidth, 0, 10.f, nameGenerator.getName()));
        if (toxicGone) addEntity(new PlanetToxicEntity(column2X, row2Y, Colors::ToxicAtmosphere(), planetWidth, planetWidth, 0, 10.f, nameGenerator.getName()));
        if (iceGone) addEntity(new PlanetIceEntity(column1X, row3Y, Colors::IceAtmosphere(), planetWidth, planetWidth, 0, 10.f, nameGenerator.getName()));
        if (lavaGone) addEntity(new PlanetLavaEntity(column2X, row3Y, Colors::LavaAtmosphere(), planetWidth, planetWidth, 0, 10.f, nameGenerator.getName()));

        if (metalGone) addEntity(new ResourceEntity(rColumn1, row4Y, Color::White(), resourceWidth, resourceWidth, "metal"));
        if (diamondGone) addEntity(new ResourceEntity(rColumn2, row4Y, Color::White(), resourceWidth, resourceWidth, "diamond"));
        if (uraniumGone) addEntity(new ResourceEntity(rColumn3, row4Y, Color::White(), resourceWidth, resourceWidth, "uranium"));
        if (antimatterGone) addEntity(new ResourceEntity(rColumn4, row4Y, Color::White(), resourceWidth, resourceWidth, "anti-matter"));

        if (playerGone) addEntity(new PlayerEntity(column1X, row5Y, Color::White(), aiWidth/2, aiHeight, 1.0f, false));
        if (mummieGone) addEntity(new MummieAIEntity(column15X, row5Y, Color::White(), aiWidth, aiHeight));
        if (snowmanGone) addEntity(new SnowmanAIEntity(column2X, row5Y, Color::White(), aiWidth, aiHeight));
